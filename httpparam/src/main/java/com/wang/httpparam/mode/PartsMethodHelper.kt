 ...
 * Description:
 */
class PartsMethodHelper(className: String, signature: String?, superName: String, config: GradleConfig) :
    MethodHelper(
        className,
        signature,
        superName,
        ...
        if (field.isNullable) {
            if (mConfig.add) {
                ...
                mv.visitFrame(
                    Opcodes.F_FULL,
                    2,
                    arrayOf<Any>(mClassName, "java/util/List"),
                    2,
                    arrayOf<Any>("java/util/List", "java/lang/String")
                )
                mv.visitVarInsn(Opcodes.ALOAD, 0)
                mv.visitFieldInsn(Opcodes.GETFIELD, mClassName, field.name, field.descriptor)
                field.toString(mv)
                ...
                mv.visitFrame(
                    ...
                    arrayOf<Any>(mClassName, "java/util/List"),
                    3,
                    arrayOf<Any>("java/util/List", "java/lang/String", "java/lang/String")
                )
                mv.visitMethodInsn(
                ...
                if (mSame) {
                    ...
                    mv.visitFrame(Opcodes.F_APPEND, 1, arrayOf<Any>("java/util/List"), 0, null)
                    mSame = true
                }
            }
        } else {
    ...
    override fun addFilesArrayField(mv: MethodVisitor, field: KField) {
        ...
        if (mArrayFirst) {
            mv.visitFrame(
                Opcodes.F_FULL,
                5,
                arrayOf<Any>(mClassName, "java/util/List", field.descriptor, Opcodes.INTEGER, Opcodes.INTEGER),
                0,
                arrayOfNulls(0)
            )
            mArrayFirst = false
    ...
    override fun toString(): String {
        return "PartsMethodHelper{mConfig=$mConfig, mClassName='$mClassName\', mClassDesc='$mClassDesc\', mClassSign='$mClassSign\', mSuperName='$mSuperName\', mReturnDesc='$mReturnDesc\', mReturnSignature='$mReturnSignature\', mSame=$mSame, methodName='$methodName\', root=$root, replace=$replace}"
    }
}